...
<div class="checkout-order__list">
    ...
    <div class="checkout-order__item">
        ...
        <div class="checkout-order-item__body">
            <a href="{{ route('product', $product['slug']) }}" class="checkout-order-item__title">{{ $product['title'] }}</a>
            <p class="checkout-order-item__article">{{ __('front.sku') }} {{ $product['sku'] }}</p>
            <div class="checkout-order-item__bottom">
                <p class="checkout-order-item__num">{{ __('checkout.count_long') }}: <span>{{ $product['quantity'] }}</span></p>
                <div class="checkout-order-item__price">{{ Number::currency(number: $product['price'] * $product['quantity'], in: 'UAH', locale: 'uk') }}</div>
            </div>
        </div>
    </div>
    @endforeach
...
<div class="checkout-order__bottom">
    ...
    <div class="checkout-order__buttons">
        <button form="checkout_form" type="submit" class="checkout-order__btn btn">{{ __('checkout.order_completed') }}</button>
    </div>
    @guest
