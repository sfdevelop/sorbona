    ...
    <div class="form-group__inputs">
        <div class="form__item">
            ...
            <select id="np-city"
                ...
                <option value="" disabled=""
                        selected="">{{ __('checkout.delivery_method_np_city_placeholder') }}</option>
                @foreach($cities as $city)
                    <option value="{{$city}}" {{ $selectedNpCity == $city ? 'selected' : '' }}>{{$city}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form__item">
            ...
            <select id="np-depot"
                ...
                @foreach($depot as $depotItem)
                    <option value="{{$depotItem}}" {{ $selectedNpDepot == $depotItem ? 'selected' : '' }}>{{$depotItem}}</option>
                @endforeach
            </select>
        </div>
        @endif
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.addEventListener('change', function(e) {
                if (e.target && e.target.id === 'np-city') {
                    ...
                    @this.set('selectedNpCity', e.target.value);
                }
                if (e.target && e.target.id === 'np-depot') {
                    console.log('Depot changed to:', e.target.value);
                    @this.set('selectedNpDepot', e.target.value);
                }
            });
